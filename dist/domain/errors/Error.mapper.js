import { z } from "zod";
import { ValidationError, PropertyRequiredError, DuplicateError, DatabaseError, } from "../errors/ApplicationError.js";
 ...
 */
export function mapZodToCustomError(zodError) {
    ...
    switch (firstError.code) {
        case "invalid_type":
            ...
            return new ValidationError(`Datos inválidos en ${firstError.path.join(".")}: se esperaba ${firstError.expected}, se recibió ${firstError.received}`);
        case "too_small":
        case "too_big":
            ...
            return new ValidationError(`El ${typeMsg} de ${firstError.path.join(".")} es inválido: debe ser ${verb} a ${limit}.`);
        case "invalid_string":
            ...
            if (firstError.path.includes("password")) {
                return new ValidationError(`La contraseña no cumple con los requisitos de seguridad.`);
            }
            return new ValidationError(`Formato inválido para ${firstError.path.join(".")}: ${firstError.message}`);
        case "custom":
            // Manejo de errores custom (ej. validaciones de duplicados a nivel Zod)
            if (firstError.message.toLowerCase().includes("duplicado") ||
        ...
        default:
            return new ValidationError(`Error de validación desconocido en ${firstError.path.join(".")}: ${firstError.message}`);
    }
}
/**
 ...
 */
export function mapDatabaseToCustomError(error, operation) {
    switch (error.code) {
        case "23505": {
            // unique_violation
        ...
        case "23503": {
            ...
            return new ValidationError(`El registro relacionado no fue encontrado o está en uso: ${error.detail || "clave foránea"}`);
        }
        case "42P01": // undefined_table
            return new DatabaseError(`Tabla inexistente: ${error.message}`, error.code, operation);
        case "42703": // undefined_column
            return new DatabaseError(`Columna inexistente: ${error.message}`, error.code, operation);
        ...
        default:
            ...
            return new DatabaseError(`Fallo en la operación de base de datos: ${error.message}`, error.code, operation);
    }
}
/**
 ...
 */
export function handlerServiceError(err, operation) {
    ...
    try {
        if (err instanceof z.ZodError) {
            ...
            customError = new DatabaseError(`Unexpected error during ${operation}: ${err.message}`, undefined, operation);
        }
    }
    catch (mappingError) {
        customError = new DatabaseError(`Fallo al procesar el error: ${mappingError.message}`, "MapperFailed", operation);
    }
